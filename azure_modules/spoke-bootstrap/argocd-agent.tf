...
EOAGENT
      ...
      RESULT=$(az aks command invoke \
        ...
        --command "helm upgrade --install argocd argo-cd --repo https://argoproj.github.io/argo-helm --version ${self.triggers.chart_version} -n ${self.triggers.namespace} --create-namespace -f compute-values.yaml --wait --atomic --timeout 15m && kubectl create configmap spiffe-helper-config -n ${self.triggers.namespace} --from-file=helper.conf=helper.conf --dry-run=client -o yaml | kubectl apply -f - && kubectl apply -f agent.yaml" \
