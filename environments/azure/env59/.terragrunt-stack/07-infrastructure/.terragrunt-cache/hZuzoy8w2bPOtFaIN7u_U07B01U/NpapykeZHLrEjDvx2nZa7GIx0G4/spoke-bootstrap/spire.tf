...
EOBUNDLE
      az aks command invoke \
        ...
        --command "kubectl create namespace ${local.spire_namespace} --dry-run=client -o yaml | kubectl apply -f - && kubectl create configmap spire-bundle -n ${local.spire_namespace} --from-file=bundle.spiffe=bundle.spiffe --dry-run=client -o yaml | kubectl apply -f - && helm upgrade --install spire spire --repo https://spiffe.github.io/helm-charts-hardened --version ${self.triggers.chart_version} -n ${local.spire_namespace} -f values.yaml --atomic --timeout 10m" \
        --only-show-errors
    EOT
  }
}
